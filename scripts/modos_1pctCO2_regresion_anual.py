...
for model in models[:]:
    ...
    try:
      ...
      gw = funciones.anomalias(gw,gw.isel(time=slice(0,int(12*50))))
      ...
      dat = funciones.anomalias(dat,dat.isel(time=slice(0,int(12*50))))
      solver = Eof(dat)
      pcs = solver.pcs()
      eofs = solver.eofs()
      a = 0; b = 0; c = 0; d = 0
      ...
      dic_tit_mapa = [model+' GW ',
                      model+' EOF1 '+str(fv[0])+'% (DJF)',
                      model+' EOF2 '+str(fv[1])+'% (DJF)',
                      model+' EOF3 '+str(fv[2])+'% (DJF)']
      ...
      fig.savefig(path+'/EOF_regress_out_GW_'+model+'_1pctCO2_wo_seasonal_cycle.png') 
    except:
